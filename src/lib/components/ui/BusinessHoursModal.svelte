...
>
    ...
    <div class="space-y-4">
        ...
        <div class="flex gap-2">
            {#each days as day}
                <button 
                    class={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-semibold transition-all ${selectedDays.includes(day.value) ? 'bg-blue-500 text-white shadow-md scale-105' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
    ...
    <div class="flex items-center justify-between py-2">
        <span class="text-sm font-bold text-gray-700">Closed</span>
        <button 
            type="button"
            class={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${isClosed ? 'bg-gray-200' : 'bg-gray-200'}`}
    ...
    <div class="space-y-4 pt-2">
        ...
        <div class="space-y-3">
            {#each timeSlots as slot, i}
                <div class="flex items-center gap-3 bg-gray-50 p-3 rounded-xl border border-gray-100">
                    <div class="flex-1 space-y-1">
                        ...
                        <div class="relative">
                            <input 
                                ...
                                class="w-full pl-3 pr-8 py-2 bg-white rounded-lg border border-gray-200 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                            />
                    ...
                    <button 
                        class="mt-5 p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"
        ...
        <button 
            onclick={addSlot}
            class="flex items-center gap-2 text-blue-600 font-semibold text-sm hover:text-blue-700 hover:bg-blue-50 px-3 py-2 rounded-lg transition-colors w-max"
        >
